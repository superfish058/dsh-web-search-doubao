...
import {
  ...
  DOUBAO_DEFAULT_EDITION,
} from './provider.ts'
import type { DoubaoSearchProviderOptions } from './provider.ts'
import type { DoubaoEdition } from './types.ts'
...
export type { DoubaoSearchProviderOptions } from './provider.ts'
export type {
  DoubaoCustomRequest,
...
export interface Config {
  /** Literal Doubao Search API key; a value stored under {@link apiKeyEnv} (the settings card) wins over it. */
  apiKey?: string
  /** Credential reference resolved for each search; defaults to `DOUBAO_SEARCH_API_KEY`. */
  apiKeyEnv?: string
  /** Endpoint base; the edition appends the operation path. Defaults to the public Doubao Search API. */
  baseURL?: string
  /** Service edition: `custom` (default, honors Count) or `global` (Parts-based bodies). */
  edition?: DoubaoEdition
  /** `global` edition only: maximum characters per document body snippet. */
  maxSnippetLength?: number
  /** `custom` edition only: source-authority floor (1 = government/top institutions … 4). */
  authLevel?: 1 | 2 | 3 | 4
}
...
export const Config: z<Config> = z.object({
  ...
  baseURL: z.string(),
  edition: z.union(['global', 'custom'] as const).default(DOUBAO_DEFAULT_EDITION),
  maxSnippetLength: z.number(),
  authLevel: z.union([1, 2, 3, 4] as const),
})
 ...
 */
function resolveOptions(ctx: Context, config: Config): DoubaoSearchProviderOptions {
  ...
  return {
    ...
    resolve: async () => {
      ...
      const stored = async (ref: string): Promise<string | undefined> => {
